...
#[tokio::test]
async fn test_bridge_connect_returns_not_implemented() {
    let mut svc = CloudBridgeService::new();
    let result = svc.connect("c-1").await;
    assert!(result.is_err());
    assert!(svc.connected_container_ids().is_empty());
}

#[tokio::test]
async fn test_bridge_always_disconnected() {
...
#[tokio::test]
async fn test_bridge_request_returns_not_implemented() {
    let mut svc = CloudBridgeService::new();
